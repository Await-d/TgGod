...
class TelegramService:
    ...
    async def initialize(self):
        ...
        if self.client is None:
            try:
                ...
                logger.info(
                    f"API ID: {api_id}, API Hash: {'*' * len(str(api_hash)) if api_hash else 'None'}"
                )

                if not api_id or not api_hash:
                    raise ValueError(
                        f"API配置不完整: API ID={api_id}, API Hash={'已设置' if api_hash else '未设置'}"
                    )
    ...
    async def reconnect(self):
        if self.client:
            try:
                ...
                logger.warning(f"断开旧Telegram连接失败: {e}")
        self.client = None
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"遇到Flood Wait，等待{wait_time}秒后重试 (尝试 {attempt + 1}/{max_retries})"
                    )
    ...
    async def get_group_info(self, group_identifier) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(entity, (Channel, Chat)):
                ...
                logger.warning(
                    f"实体 {group_identifier} 不是群组或频道，类型: {type(entity)}"
                )
                return None
    ...
    async def add_group_to_db(
        self, group_username: str, db: Session
        ...
        try:
            ...
            existing_group = (
                ...
                .filter(TelegramGroup.username == group_username)
                .first()
            )
    ...
    async def get_messages(
        self, group_identifier, limit: int = 200, offset_id: int = 0
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if message.forward:
                ...
                try:
                    ...
                    # 记录转发链长度（防止无限转发链）
                    message_data["forward_chain_length"] = getattr(
                        message.forward, "chain_length", 1
                    )
    ...
    async def _process_media(
        self, message: Message, download: bool = True
    ) -> Dict[str, Any]:
        """处理媒体文件
        ...
        try:
            if isinstance(message.media, MessageMediaPhoto):
                ...
                if hasattr(message.media.photo, "file_reference"):
                    media_info["media_file_unique_id"] = str(
                        message.media.photo.file_reference
                    )
                ...
                if download:
                    try:
                        ...
                        # 保存相对路径
                        media_info["media_path"] = (
                            f"media/{media_type}s/{original_filename}"
                        )
                        media_info["media_downloaded"] = True
                    ...
                    logger.debug(
                        f"跳过转发{media_info['media_type']}文件下载: {original_filename}"
                    )
    ...
    async def download_media(
        self, message: Message, download_path: str
    ) -> Optional[str]:
        """下载媒体文件"""
        try:
            await self.initialize()
    ...
    async def save_messages_to_db(
        self, group_id: int, messages: List[Dict[str, Any]], db: Session
    ) -> int:
        """保存消息到数据库，返回保存的消息数量"""
        try:
            saved_count = 0
            ...
            for message_data in messages:
                ...
                # 创建消息记录
                message = TelegramMessage(group_id=group_id, **cleaned_data)
                db.add(message)
                saved_count += 1
    ...
    async def sync_group_messages(
        ...
        try:
            ...
            with optimized_db_session() as db:
                group = (
                    db.query(TelegramGroup).filter(TelegramGroup.id == group_id).first()
                )
                if not group:
            ...
            logger.info(
                f"开始同步群组 {group.title} 的消息，时间范围: {start_date} - {end_date}"
            )

            # 获取时间范围内的消息
            messages = await self._get_messages_by_time_range(
    ...
    async def sync_messages_by_month(
        self, group_identifier, months: List[Dict[str, Any]], group_id: int = None
    ) -> Dict[str, Any]:
        """按月同步群组消息
        ...
        try:
            ...
            if group_id is not None:
                ...
                # 首先检查缓存中是否有群组信息
                cached_group = telegram_cache.chat_cache.get(
                    "group_db_mapping", entity.id
                )
                if cached_group:
            ...
            try:
                    ...
                    try:
                        ...
                        if not year or not month:
                            ...
                            sync_result["failed_months"].append(
                                {"month": month_info, "error": "月份信息不完整"}
                            )
                            continue
                        ...
                        if group_id:
                            try:
                                ...
                                # 向所有连接的客户端发送进度更新
                                connected_clients = (
                                    websocket_manager.get_connected_clients()
                                )
                                logger.info(
                                    f"向 {len(connected_clients)} 个客户端发送进度更新"
                                )
                        ...
                        if group_id is None:
                            ...
                            sync_result["failed_months"].append(
                                {"month": month_info, "error": "未找到群组记录"}
                            )
                            continue
                        ...
                        saved_count = 0
                        try:
                            with optimized_db_session(
                                autocommit=True, max_retries=3
                            ) as db:
                        ...
                        logger.info(
                            f"✓ {year}-{month:02d} 同步完成: {saved_count}/{len(month_messages)} 条消息"
                        )

                    except Exception as e:
                        logger.error(f"同步 {year}-{month:02d} 失败: {e}")
                        sync_result["failed_months"].append(
                            {"month": month_info, "error": str(e)}
                        )
                ...
                logger.info(
                    f"按月同步完成: 总计 {sync_result['total_messages']} 条消息"
                )
                return sync_result
    ...
    async def _get_messages_by_time_range(
        self, entity, start_date: datetime, end_date: datetime
    ) -> List[Dict[str, Any]]:
        """根据时间范围获取消息"""
        try:
            messages = []
            ...
            logger.info(
                f"获取到 {len(messages)} 条消息 ({start_date.strftime('%Y-%m')})"
            )
            return messages
    ...
    async def get_default_sync_months(self, count: int = 3) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(count):
                ...
                months.append({"year": target_date.year, "month": target_date.month})
    ...
    async def send_message(
        self, group_username: str, text: str, reply_to_message_id: Optional[int] = None
    ) -> Optional[int]:
        """发送消息到群组"""
        try:
            await self.initialize()
            ...
            message = await self.client.send_message(
                entity, text, reply_to=reply_to_message_id
            )
    ...
    async def send_media_message(
        ...
        for index, upload_file in enumerate(files):
            ...
            file_stream.name = upload_file.filename or f"upload_{index}"
    ...
    async def edit_message(
        self, group_username: str, message_id: int, new_text: str
    ) -> bool:
        """编辑群组消息"""
        try:
            await self.initialize()
    ...
    async def _process_forward_source(self, forward_info):
        ...
        try:
            ...
            if forward_info.from_name:
                ...
                # 使用缓存安全获取用户信息
                user_info = await telegram_cache.get_user_safe(
                    self.client, forward_info.from_id
                )
                if user_info:
                    result["forwarded_from"] = (
                        user_info["full_name"]
                        or user_info["username"]
                        or f"用户{user_info['id']}"
                    )
                else:
                    # 尝试直接从Telegram获取用户信息
                    try:
                        ...
                        logger.warning(
                            f"无法获取转发用户信息 {forward_info.from_id}: {e}"
                        )
                ...
                if isinstance(chat, Channel):
                    result["forwarded_from_type"] = (
                        "channel" if chat.broadcast else "supergroup"
                    )
                    # 获取频道签名信息
                    if (
                        hasattr(forward_info, "post_author")
                        and forward_info.post_author
                    ):
            ...
            result.update(
                {"forwarded_from": "处理错误", "forwarded_from_type": "error"}
            )
    ...
    async def _query_group_with_optimization(self, telegram_id):
        ...
        try:
            ...
            # 1. 检查内存缓存
            cached_result = telegram_cache.chat_cache.get(
                "optimized_group_query", cache_key
            )
            if cached_result:
            ...
            with optimized_db_session(autocommit=False, max_retries=3) as db:
                ...
                if group_record:
                    ...
                    logger.warning(
                        f"数据库中未找到telegram_id为 {telegram_id} 的群组记录"
                    )
                    return None
            ...
            if forward_info.chat:
                ...
                if isinstance(chat, Channel):
                    try:
                        ...
                        # 检查是否禁止转发
                        if (
                            hasattr(full_channel.full_chat, "noforwards")
                            and full_channel.full_chat.noforwards
                        ):
                            return False
